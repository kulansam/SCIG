...
def pesudobulk_cigpred_mouse (inputtype_pred,exp_filename_pred,alll_seq_features1,gene_name_length1,training_table_mouse):
    ...
    if inputtype_pred =='rawcount':
        print("Unormalised expression rawcounts are processing")
    ...
    for i in ten_celltype_train:
        ...
        combine_feat = combine_feat[['Geneid','TPM_exon_exp_mean', 'mca3gini', 'mtdtau_log',
...
def pesudobulk_cig_reg_pred_mouse (cig_pred_result,all_db_grn_mouse,tf_mouse,training_table_cigreg_mouse,celltype_names):
    ...
    for c_type in celltype:
        ...
        master_tf_cig_final.columns=['Geneid','TF','no_of_parent_edges', 'no_of_children_edges','CIG', 'EXP','CIG_distance','CIGscore_parent_mean','Exp_parent_mean','CIG_distance_parent_mean','CIGscore_parent_median','Exp_parent_median','CIG_distance_parent_median','CIGscore_parent_CV','Exp_parent_CV','CIG_distance_parent_CV',
        ...
        pred = pd.DataFrame(pred)
        #pred.columns=[c_type]
        pred.columns=[c_type+'_MasterTF_probability']
